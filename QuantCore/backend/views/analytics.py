...
class StockDataAnalysisAPIView(APIView):
    ...
    def get(self, request):
        ...
        if df_daily.empty or len(df_daily) < 90:
            return Response({'error': 'Insufficient daily historical data.'}, status=400)
    ...
    # ---------- 5) Evaluate combined (hybrid) model on historical test set ----------
        ...
        if latest_row[baseline_features].isnull().any(axis=None):
            return Response({'error': 'Not enough history in daily data to compute baseline features.'}, status=400)
